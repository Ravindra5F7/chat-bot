...
def chat():
    ...
    if query.lower() in greetings:
        logger.info(f"Greeting detected for session {session_id if session_id else 'N/A'}: '{query}'")
        return jsonify({
            "answer": "Hello! How can I help you today?",
            "session_id": session_id or str(uuid.uuid4()),
            "references": [],
            "thinking": "Recognized user greeting, bypassed RAG/LLM.",
        }), 200
    ...
    try:
        ...
        bot_message_id = None
        try:
            bot_message_id = database.save_message(session_id, 'bot', bot_answer, references, thinking_content)
